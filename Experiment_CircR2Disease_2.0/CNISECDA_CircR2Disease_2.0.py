...
CD_association_matrix = pd.read_csv("../Dataset/CircR2Disease_2.0/Association Matrix.csv", index_col=0)
circRNA_similarity_fusion_matrix = pd.read_csv('../Dataset/CircR2Disease_2.0/circRNA_similarity_fusion_matrix.csv', index_col=0)
disease_similarity_fusion_matrix = pd.read_csv('../Dataset/CircR2Disease_2.0/disease_similarity_fusion_matrix.csv', index_col=0)
...
NEGATIVE_SAMPLE_CHA_ALL = np.array(NEGATIVE_SAMPLE_CHA_ALL, dtype=np.float32)
...
for i in range(5):
    ...
    # CatBoost
    catboost = CatBoostClassifier(iterations=20, learning_rate=0.001, verbose=0, depth=4, random_seed=36)
