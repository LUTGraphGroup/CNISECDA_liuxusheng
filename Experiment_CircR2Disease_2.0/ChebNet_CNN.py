...
class ChebNet_CNN_DASE(nn.Module):
    def __init__(self, in_circfeat_size, in_disfeat_size, outfeature_size, drop_rate, features_embedding_size, negative_times):
        ...
        self.cnn_layer1 = nn.Sequential(
            nn.Conv2d(in_channels=1, out_channels=6, kernel_size=(1, 1), padding=0),  # 二维卷积层，对数据进行卷积操作
    ...
    def forward(self, graph, circ_feature_tensor, dis_feature_tensor, association_matrix, train_model):
        ...
        circ_circ_f = circ_feature_tensor.mm(self.W_circ)
        dis_dis_f = dis_feature_tensor.mm(self.W_dis)
        N = circ_circ_f.size()[0] + dis_dis_f.size()[0]

        h_m_d_feature = torch.cat((circ_circ_f, dis_dis_f), dim=0)
...
if __name__ == '__main__':
    ...
    circRNA_similarity_fusion_matrix = pd.read_csv('../Dataset/CircR2Disease_2.0/circRNA_similarity_fusion_matrix.csv',
                                                   index_col=0)
    disease_similarity_fusion_matrix = pd.read_csv('../Dataset/CircR2Disease_2.0/disease_similarity_fusion_matrix.csv',
                                                   index_col=0)
